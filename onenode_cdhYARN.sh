...
cdhrelease=${1:-5.0.0b1}
major=$(echo $cdhrelease | /bin/cut -d"." -f1)

# Declare array for the node roles
declare -a noderoles=(hdfs-namenode hdfs-secondarynamenode hdfs-datanode yarn-nodemanager yarn-resourcemanager yarn-proxyserver mapreduce mapreduce-historyserver conf-pseudo)
...
# Packages for Cloudera's Distribution for Hadoop, Version ${major}, on RedHat	or CentOS 6 x86_64
name=Cloudera's Distribution for Hadoop, Version ${major}
...
##### 5. Create /tmp and other directories in HDFS
	/usr/bin/sudo -u hdfs hadoop fs -rm -r /tmp
	/usr/bin/sudo -u hdfs hadoop fs -mkdir -p /tmp/hadoop-yarn/staging/history/done_intermediate
	/usr/bin/sudo -u hdfs hadoop fs -chown -R mapred:mapred /tmp/hadoop-yarn/staging 
	/usr/bin/sudo -u hdfs hadoop fs -chmod -R 1777 /tmp 
	/usr/bin/sudo -u hdfs hadoop fs -mkdir -p /var/log/hadoop-yarn
	/usr/bin/sudo -u hdfs hadoop fs -chown yarn:mapred /var/log/hadoop-yarn
